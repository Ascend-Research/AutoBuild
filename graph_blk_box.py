...
if __name__ == "__main__":
    ...
    block_names = []
    all_data = []
    for key, block_type in block_dict.items():
        block_names.append(key)
        print("   ==>", block_type)
        sel_entries = get_relevant_entries(score_dict, block_type)
    ...
    plt.title(f"{arch} {params.title} Block Index Scores")

    plt.tight_layout()
    plt.savefig(P_SEP.join([PLOTS_DIR, "_".join([arch_f, "graph_block_box", params.title])]))
