...
from search.rm_search.ofa_profile.arch_gen import sample_ofa_mbv3_stage_configs
from search.rm_search.ofa_profile.arch_mutator import OFAStageWholeMutator
from search.rm_search.ofa_profile.search_utils import get_mbv3_stage_level_mutate_probs
from search.rm_search.constants import DIR_TO_APPEND
import pickle


sys.path.append(DIR_TO_APPEND)
from iterators.ofa_manager import MBv3Manager
...
def main(params):
    ...
    # Load lat predictor
    lat_predictor = MBv3Manager(params.metric)
    ...
    if params.unit_corpus is not None:
        ...
        from iterators.mbv3 import gen_sgs
        subgraph_list = gen_sgs(units=[1])
        sgs = [x['config'] for x in subgraph_list]
        stage_units = [sgs, sgs, sgs, sgs, sgs]
    ...
    if params.pareto_init_checkpoint is None:
        ...
        while len(init_net2perfs) < params.random_init_set_size:
            net_configs = sample_ofa_mbv3_stage_configs(stage_units)
