...
"""
OFA-MBV3 random mutation constrained/un-constrained acc search
"""


def prepare_local_params(parser):
    parser.add_argument("-model_name", required=False, type=str,
                        default="ofa_mbv3_rm_cons_acc")
...
def main(params):
    ...
    # Load lat predictor
    lat_predictor = None #get_mbv3_search_lat_predictor(params.lat_predictor_type,
                         #                         params.resolution,
    ...
    target_nets = MBV3_CONS_ACC_SEARCH_TARGET_NETS
    book_keeper.log("Specified {} target net(s)".format(len(MBV3_CONS_ACC_SEARCH_TARGET_NETS)))
    for c in MBV3_CONS_ACC_SEARCH_TARGET_NETS:
    ...
    elif len(MBV3_CONS_ACC_SEARCH_TARGET_NETS) > 0:
        max_cons_score = min([lat_predictor(_cfg) for _cfg in MBV3_CONS_ACC_SEARCH_TARGET_NETS])
